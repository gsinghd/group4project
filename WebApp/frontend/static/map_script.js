...
function initMap() {
    ...
    fetch('/static/data/statesData.geojson')
        ...
        .then(geojsonData => {
            fetch('/state-industry-growth-data')
                .then(response => response.json())
                .then(growthData => {
                    console.log("Aggregated State Growth Data:", growthData);
...
function getColor(growth) {
    ...
    const value = parseFloat(growth)*100;
    const red = value < 0 ? 255 * Math.min(1, -value / 5) : 0;
    const green = value > 0 ? 255 * Math.min(1, value / 5) : 0;
...
function initMap() {
    ...
    loadGrowthData('/state-industry-growth-data'); // Default load
}
